...
feelsLikeDisplay.textContent = `Ощущаеться как ${currentWeatherData.main.feels_like}°C`;
...
const date = new Date().toLocaleDateString('ru',{ weekday: 'short', day:'numeric', month:'short', });
...
forecastData.list.forEach((item, i) => {
    ...
    labels.push(date.toLocaleDateString("ru", {
        month: 'short',
        day: 'numeric'
    }));
...
addListeners();
getWeatherData("Набережные челны");
